...
def test_rsa_encryption():
    ...
    aes_key = encrypt_folder(folder_name, "xd")
...
def test_rsa_decryption():
    ...
    decrypt_folder(folder_name, aes_key, "xd")
...
def main():
    ...
    def post_encrypt_decrypt(op):
        if op == "e":
            ...
            content = simpledialog.askstring("AES key name", "Enter name for your encrypted AES key:")
            if content == "":
                content = "content"
    ...
    search_key_button = tk.Button(window, text="Choose AES key file", command=choose_key)
...
# Example of usage *terminal version*:
#
# ENCRYPTION
# Choose number from the menu: 1
# Enter non-absolute path of the folder
# folder_path = 'example'
#
# DECRYPTION
